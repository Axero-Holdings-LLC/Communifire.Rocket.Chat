...
export default class SlackAdapter {
	...
	**/
	...
	copyChannelInfo(rid, channelMap) {
		slackLogger.debug('Copying users from Slack channel to Communifire', channelMap.id, rid);
	...
	importMessages(rid, callback) {
		...
		if (rocketchat_room) {
			if (this.getSlackChannel(rid)) {
				...
				slackLogger.debug('Importing messages from Slack to Communifire', this.getSlackChannel(rid), rid);
				...
				slackLogger.debug('Pinning Slack channel messages to Communifire', this.getSlackChannel(rid), rid);
		...
		slackLogger.error('Could not find Communifire room with specified id', rid);
